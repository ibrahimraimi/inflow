    ...
    <div className="lg:mt-8 mt-10 space-y-6">
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 border-b pb-6">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight">Conversion Funnels</h1>
          <p className="text-muted-foreground">
            Analyze drop-off rates across key user journeys.
          </p>
        </div>
        <Button onClick={() => router.push(`/dashboard/${websiteId}/funnels/new`)} className="cursor-pointer">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {funnels?.map((funnel) => (
          <Card key={funnel.id} className="cursor-pointer hover:border-primary/50 transition-colors" onClick={() => router.push(`/dashboard/${websiteId}/funnels/${funnel.id}`)}>
        ...
        {!funnels?.length && (
          <div className="col-span-full border-2 border-dashed rounded-lg p-12 text-center text-muted-foreground">
            <h3 className="text-lg font-medium mb-2 text-foreground">No funnels found</h3>
            <p className="mb-6">Create a funnel to start tracking conversion rates across your website.</p>
            <Button variant="outline" onClick={() => router.push(`/dashboard/${websiteId}/funnels/new`)}>
